...
// Dimension du svg 
const margin = {
    top:80, right:60,bottom:70,left:70
},
...
function createLegendNbHeures(){
    const sizeLegend = d3.select("#size-hour-legend");
    const sizes = [2,4,8];
...
function createLegendAnxiete(){
    const sizeLegend = d3.select("#size-anxiety-legend");
    const sizes = [2,4,8];
    const labels = ["Faible", "Moyen", "Elevé"];
    const sizeSvg = sizeLegend.append("svg")
        .attr("width", 150)
...
// Création des axes et labels du tableau
function drawAxes(){
    svg.append("g")
...
function createScatterPlot(data){
    const points = groupePoints.selectAll(".data-point")
        .data(data, d => d.Name)
        .join(
            enter => {
                    ...
                    infoBulle
                        ...
                        .html(`
                            ...
                            <div style="font-size:13px">
                              <div><strong>Activité:</strong> ${d.Phone_Usage_Purpose}</div>
                              <div><strong>Temps:</strong> ${d.Daily_Usage_Hours} h</div>
                              <div><strong>Performance:</strong> ${d.Academic_Performance}%</div>
                            </div>
                        `);
...
// Create a pair of range sliders for Anxiety filter and append under the anxiety legend
function createFilterAnxiete() {
    ...
    const container = d3.select('#size-anxiety-legend');
    const wrap = container.append('div').attr('class', 'filter-anxiety').style('margin-top','10px');

    wrap.append('label').text('Filtrer niveau d\'anxiété :').style('display','block').style('font-size','0.9em');
    const controls = wrap.append('div').style('display','flex').style('gap','8px').style('align-items','center');
    ...
    // clearer display with labels and formatted numbers
    const display = wrap.append('div').style('font-size','0.85em').style('margin-top','6px');
    display.append('span').attr('class','label-min').text('Min: ');
    display.append('span').attr('id','anx-display-min').text(min.toFixed(1));
    display.append('span').text('   ');
    display.append('span').attr('class','label-max').text('Max: ');
    display.append('span').attr('id','anx-display-max').text(max.toFixed(1));
...
// Create a pair of range sliders for Daily Usage Hours and append under the hours legend
function createFilterNbHeures() {
    ...
    const container = d3.select('#size-hour-legend');
    const wrap = container.append('div').attr('class', 'filter-hours').style('margin-top','10px');

    wrap.append('label').text('Filtrer heures quotidiennes :').style('display','block').style('font-size','0.9em');
    const controls = wrap.append('div').style('display','flex').style('gap','8px').style('align-items','center');
    ...
    const display = wrap.append('div').style('font-size','0.85em').style('margin-top','6px');
    display.append('span').attr('class','label-min').text('Min: ');
    display.append('span').attr('id','hours-display-min').text(min.toFixed(1));
    display.append('span').text('   ');
    display.append('span').attr('class','label-max').text('Max: ');
    display.append('span').attr('id','hours-display-max').text(max.toFixed(1));
...
function filterData() {
    ...
    const groups = groupePoints.selectAll(".data-point");
...
d3.csv("./data/data.csv").then(function(data) {
    const limited_data = data.slice(0, 250);
    ...
    // create range filters under legends
    createFilterAnxiete();
